...
export async function classifyUserQuery(query: string): Promise<{
  userquery: string;
  classifications: { department: string; email: string | null; subquery: string | null }[];
  raw?: AIResponse;
}> {
  try {
    ...
    const classifications: { department: string; email: string | null; subquery: string | null }[] = [];
      ...
      if (departments.length > 0) {
        if (Array.isArray(departments[0])) {
          for (const item of departments) {
            if (Array.isArray(item) && typeof item[0] === "string") {
              classifications.push({
                ...
                subquery: typeof item[2] === "string" ? (item[2] as string) : null,
              });
            }
          }
          ...
          classifications.push({
            ...
            email: typeof departments[1] === "string" ? (departments[1] as string) : null,
            subquery: typeof departments[2] === "string" ? (departments[2] as string) : null,
          });
        }
      }
    }
    ...
    if (classifications.length === 0) {
      console.warn("AI classification parsing failed; no valid departments found.");
    } else {
      console.log(`Parsed AI classified departments: ${classifications.map(c => c.department).join(", ")}`);
    }
